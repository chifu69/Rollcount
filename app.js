...
let data=load(KEY,{}),history=load(HKEY,[]),scrap=String(load(SKEY,'')),consumption=String(load(CKEY,''));
...
function reportText(){const rows=inStock(),gt=rows.reduce((s,x)=>s+x.total,0),sv=scrapN();const divider='------------------------------';const ct=consumptionText();return ['ROLL COUNT REPORT',dateLong(),divider,...rows.map(x=>`${x.type.padEnd(12,' ')} ${String(x.total).padStart(3,' ')} rolls`),divider,`TOTAL ROLLS: ${gt}`,`SCRAP: ${sv.toLocaleString()}`,...(ct?['',ct]:[])].join('\n')}
